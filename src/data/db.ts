...
function runSchemaMigrations() {
  ...
  while (current < SCHEMA_VERSION) {
    ...
    if (!migration) {
      console.error(
        `[SQLite] Missing schema migration from v${current} to v${current + 1}. ` +
    ...
    try {
      ...
      console.error(`[SQLite] Schema migration v${current} → v${current + 1} failed:`, error);
      Alert.alert(
        'Error de migración',
...
export function initDatabase() {
  ...
  if (currentDataset !== DATASET_VERSION) {
    ...
    console.log(`[SQLite] Dataset up-to-date (v${DATASET_VERSION}, ${count} rows).`);
  }
}
