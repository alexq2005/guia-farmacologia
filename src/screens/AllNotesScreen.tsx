        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              Escribe notas en el detalle de cualquier fármaco
            </Text>
          </View>
        }
        renderItem={({ item }) => {
          const drug = getDrugById(item.drugId);
          const unitColor = drug
            ? UNIT_COLORS[drug.unidadId] || colors.primary
            : colors.primary;
          return (
            <TouchableOpacity
              style={[styles.noteCard, { borderLeftColor: unitColor }]}
              onPress={() =>
                navigation.navigate('DrugDetail', { drugId: item.drugId })
              }
              activeOpacity={0.7}
              accessibilityRole="button"
              accessibilityLabel={`Nota sobre ${drug?.nombre || item.drugId}`}
            >
              <View style={styles.noteHeader}>
                <Text style={styles.noteDrug} numberOfLines={1}>
                  {drug?.nombre || item.drugId}
                </Text>
                <TouchableOpacity
                  onPress={() =>
                    handleDelete(item.drugId, drug?.nombre || item.drugId)
                  }
                  hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
                  accessibilityRole="button"
                  accessibilityLabel={`Eliminar nota de ${
                    drug?.nombre || item.drugId
                  }`}
                >
                  <MaterialCommunityIcons
