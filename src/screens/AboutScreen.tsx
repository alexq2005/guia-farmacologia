...
export function AboutScreen() {
  ...
  const handleActivate = useCallback(async () => {
    ...
    if (success) {
      ...
      Alert.alert(
        'Activado',
        'Todas las funciones han sido desbloqueadas permanentemente.',
      );
    } else {
  ...
  const handleEmail = () => {
    Linking.openURL(
      `mailto:${CONTACT_EMAIL}?subject=Guía Farmacológica - Contacto`,
    );
  };
  ...
  const stats = [
    { icon: 'pill', value: getDrugCount().toString(), label: 'Fármacos' },
    {
      ...
      value: pathologies.length.toString(),
      label: 'Patologías',
    },
    {
      icon: 'chart-timeline-variant-shimmer',
      value: scalesData.length.toString(),
      label: 'Escalas',
    },
    {
      icon: 'flask-outline',
      value: labValuesData.length.toString(),
      label: 'Lab',
    },
    {
      icon: 'alert-octagon',
      value: protocolsData.length.toString(),
      label: 'Protocolos',
    },
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.cardText}>
            Guía de consulta rápida para profesionales y estudiantes de
            enfermería. Incluye fármacos, patologías, escalas clínicas
            interactivas, valores de laboratorio, protocolos de emergencia,
            calculadoras, interacciones y cuidados de enfermería.
          </Text>
          <Text style={styles.cardText}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.featureList}>
            {[
              {
                icon: 'magnify',
                text: 'Búsqueda inteligente con coincidencia sin acentos',
              },
              {
                icon: 'star-outline',
                text: 'Sistema de favoritos con persistencia local',
              },
              {
                icon: 'note-text-outline',
                text: 'Notas personales por fármaco con autoguardado',
              },
              {
              ...
              },
              {
                ...
                text: 'Verificador de interacciones medicamentosas',
              },
              {
                icon: 'iv-bag',
                text: 'Guía parenteral (Son Espases) con compatibilidades',
              },
              {
                icon: 'moon-waning-crescent',
                text: 'Modo oscuro con 3 opciones (claro/oscuro/sistema)',
              },
              {
                icon: 'share-variant-outline',
                text: 'Compartir información de fármacos',
              },
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.cardText}>
            La información farmacológica ha sido recopilada y verificada a
        ...
        {/* Estado de revisión por dataset */}
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Revisión y actualización</Text>
          <Text style={styles.cardText}>
            Cada conjunto de datos tiene su propia fecha de edición técnica y
            estado de revisión clínica. Una "edición técnica" significa que el
            archivo JSON fue modificado; una "revisión clínica" significa que un
            profesional de la salud verificó el contenido contra fuentes
            primarias y firmó el visto bueno.
          </Text>
          <View style={styles.metaList}>
            {getAllDatasetKeys().map(key => {
              const m = getMeta(key);
              ...
              return (
                <View key={key} style={styles.metaRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.metaName}>{m.displayName}</Text>
                    <Text style={styles.metaDetail}>
                      Edición: {formatMonthYear(m.lastEdited)} · Fuente:{' '}
                      {m.sourceCanonical}
                    </Text>
                  </View>
                  <View
                  ...
                  >
                    <Text
                      style={[
                        ...
                        { color: isReviewed ? colors.success : colors.warning },
                      ]}
                    >
          ...
          {(() => {
            ...
            return r.name ? (
              ...
              >
                Responsable de contenido: {r.name}
              ...
              >
                Responsable de contenido: pendiente de asignación. Hasta que un
                profesional de la salud firme la revisión, los datos se ofrecen
                como referencia educativa con la advertencia de abajo.
              </Text>
            );
          })()}
        </View>
        ...
        <View style={[styles.card, styles.disclaimerCard]}>
          ...
          >
            <MaterialCommunityIcons
              ...
              style={{ marginRight: 6 }}
            />
            <Text style={[styles.disclaimerTitle, { marginBottom: 0 }]}>
              Aviso médico importante
            </Text>
          </View>
          <Text style={styles.disclaimerText}>
            Esta aplicación es una herramienta de CONSULTA EDUCATIVA y
            REFERENCIA RÁPIDA. No constituye consejo médico, diagnóstico ni
            tratamiento, ni es un dispositivo médico.
          </Text>
          <Text
            style={[styles.disclaimerText, { fontWeight: '700', marginTop: 6 }]}
          >
            SIEMPRE verifique la información con fuentes primarias y los
            protocolos de su institución antes de administrar cualquier
            medicamento.
          </Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.cardText}>
            ¿Encontraste un error? ¿Tenés sugerencias o comentarios? No dudes en
            escribirnos.
          </Text>
          <TouchableOpacity
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>
            {isFreeBuild ? 'Legal' : 'Legal y suscripción'}
          </Text>
          {!isFreeBuild && (
            <TouchableOpacity
