...
export function PathologiesScreen({ navigation }: Props) {
  ...
  const renderItem = useCallback(
    ({ item: pathology }: { item: Pathology }) => (
      <TouchableOpacity
        style={[
          styles.pathologyCard,
          { borderLeftColor: PATHOLOGY_COLORS[pathology.categoria] },
        ]}
        onPress={() =>
          navigation.navigate('PathologyDetail', { pathologyId: pathology.id })
        }
        activeOpacity={0.7}
      >
        <View style={styles.cardHeader}>
          <MaterialCommunityIcons
            name={
              PATHOLOGY_ICONS[pathology.categoria] || 'clipboard-text-outline'
            }
            size={28}
            color={PATHOLOGY_COLORS[pathology.categoria]}
          ...
          <View style={styles.drugCountBadge}>
            <Text style={styles.drugCountText}>
              {pathology.farmacosRelacionados.length}
            </Text>
        ...
        <View style={styles.cardFooter}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            />
            <Text style={[styles.cardAlarmCount, { marginLeft: 3 }]}>
              {pathology.criteriosAlarma.length} criterios de alarma
            </Text>
          </View>
          <MaterialCommunityIcons
            name="chevron-right"
          ...
          {ALL_CATEGORIES.map(cat => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  ...
                  selectedCategory === cat && {
                    backgroundColor: PATHOLOGY_COLORS[cat],
                  },
                ]}
                onPress={() =>
              ...
              >
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  <MaterialCommunityIcons
                    name={PATHOLOGY_ICONS[cat] || 'clipboard-text-outline'}
                    size={14}
                    color={
                      selectedCategory === cat
                        ? '#FFFFFF'
    ...
    <Animated.View style={[styles.container, { opacity: fadeIn }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>
          {pathologies.length} patologías con fármacos vinculados
        </Text>
