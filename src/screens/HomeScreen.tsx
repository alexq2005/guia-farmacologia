...
export function HomeScreen({ navigation }: Props) {
  ...
  const [heroOverride, setHeroOverride] = useState<Drug | null>(null);
  ...
  const onRefresh = React.useCallback(() => {
    setRefreshing(true);
    // Pull-to-refresh siempre descubre un fármaco nuevo (modo "Fármaco del Día")
    const random = getRandomDrug();
    if (random) setHeroOverride(random);
    setTimeout(() => setRefreshing(false), 600);
  }, [getRandomDrug]);

  // ROADMAP v1.2: si hay actividad reciente, la hero card pasa a
  // "Continuar donde quedaste" con el último fármaco consultado.
  const mostRecentDrug =
    recentDrugs.length > 0 ? getDrugById(recentDrugs[0]) : undefined;
  const heroDrug = heroOverride || mostRecentDrug || dailyDrug;
  const isContinueHero = !heroOverride && Boolean(mostRecentDrug);
  // Evitar duplicar el fármaco de la hero card en la fila de recientes
  const recentRowIds = isContinueHero
    ? recentDrugs.slice(1, 11)
    : recentDrugs.slice(0, 10);
  ...
  const quickActions = [
    ...
    },
    {
      ...
      image: require('../assets/images/units/quiz.jpg'),
    ...
    },
    {
      ...
      image: require('../assets/images/units/nursing.jpg'),
      onPress: () => navigation.navigate('ClinicalScales'),
    },
    {
      label: 'Lab',
      gradient: ['#10B981', '#059669'] as [string, string],
      image: require('../assets/images/units/lab.jpg'),
      onPress: () => navigation.navigate('LabValues'),
    },
  ];

  const latestNotes = recentNotes(5);
  const unitColor = heroDrug
    ? UNIT_COLORS[heroDrug.unidadId] || colors.primary
    : colors.primary;
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Animated.View
          style={[styles.quickActions, { opacity: stagger[0] || 1 }]}
        >
        ...
        {!isFreeBuild && !isSubscribed && !isCodeActivated && (
          ...
          >
            ...
            <View style={styles.flex1Ml12}>
              ...
              >
                {isTrialActive
                  ? `${trialDaysLeft} días de prueba Premium`
                  : 'Prueba Premium expirada'}
              </Text>
              <Text style={styles.trialBannerSubtitle}>
                {isTrialActive
                  ? 'Todas las funciones desbloqueadas'
                  : 'Suscríbete para recuperar acceso'}
              </Text>
            </View>
            <MaterialCommunityIcons
              name="chevron-right"
        ...
        {/* Hero Card — "Continuar donde quedaste" si hay actividad reciente,
            "Fármaco del Día" si no (o tras pull-to-refresh) */}
        {heroDrug && (
          <Animated.View style={[styles.section, { opacity: stagger[1] || 1 }]}>
            <Text style={styles.sectionTitle}>
              {isContinueHero ? 'Continuar donde quedaste' : 'Fármaco del Día'}
            </Text>
            <TouchableOpacity
              style={styles.dailyHero}
              onPress={() =>
                navigation.navigate('DrugDetail', { drugId: heroDrug.id })
              }
              activeOpacity={0.8}
              accessibilityRole="button"
              accessibilityLabel={`${
                isContinueHero ? 'Continuar con' : 'Fármaco del día:'
              } ${heroDrug.nombre}`}
            >
              <ImageBackground
                source={getUnitImage(heroDrug.unidadId) || HERO_IMAGE}
                ...
                >
                  <View style={styles.dailyHeroTop}>
                    ...
                    <View style={styles.dailyViasRow}>
                      {heroDrug.viaAdministracion.slice(0, 3).map(via => (
                  ...
                  <View style={styles.dailyHeroBottom}>
                    <Text style={styles.dailyHeroName}>{heroDrug.nombre}</Text>
                    <Text style={styles.dailyHeroGeneric}>
                      {heroDrug.nombreGenerico}
                    </Text>
                    <Text style={styles.dailyHeroFamily}>
                      {heroDrug.familia}
                    </Text>
        ...
        {recentRowIds.length > 0 && (
          <Animated.View style={[styles.section, { opacity: stagger[2] || 1 }]}>
            ...
            >
              {recentRowIds.map(drugId => {
                const recentDrug = getDrugById(drugId);
                if (!recentDrug) return null;
                const rColor =
                  UNIT_COLORS[recentDrug.unidadId] || colors.primary;
                return (
                  <TouchableOpacity
                    key={drugId}
                  ...
                  >
                    ...
                    <Text style={styles.recentGeneric} numberOfLines={1}>
                      {recentDrug.nombreGenerico}
                    </Text>
                    <Text style={[styles.recentFamily, { color: rColor }]}>
                      {recentDrug.familia}
                    </Text>
                  </TouchableOpacity>
                );
            ...
            >
              ...
              >
                <View style={styles.progressRow}>
                  ...
                  <View style={styles.progressItem}>
                    <Text style={styles.progressNumber}>
                      {quizResults.reduce((s, r) => s + r.correctAnswers, 0)}
                    </Text>
              ...
              {favorites.slice(0, 10).map(favId => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={styles.recentGeneric} numberOfLines={1}>
                      {favDrug.nombreGenerico}
                    </Text>
                    <Text style={[styles.recentFamily, { color: fColor }]}>
                      {favDrug.familia}
                    </Text>
                  </TouchableOpacity>
                );
        ...
        <View style={styles.footer}>
          ...
          <TouchableOpacity
            onPress={() =>
              Linking.openURL(
                'mailto:alexq2005@gmail.com?subject=Guía Farmacológica - Contacto',
              )
            }
